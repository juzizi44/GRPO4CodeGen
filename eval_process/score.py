...
class RealTimeRewardRunner:
    def __init__(self, 
        ...
        self.save_path = None
        # 初始化两个评分器
        self.code_comment_scorer = CodeCommentScorer()
        self.llm_comment_scorer = LLMCommentScorer()
    ...
    def process_question(self, question_id: int, overall_log, idx=None) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 计算两种注释得分
                code_comment_score = self.code_comment_scorer.score_single_completion(response_code)
                llm_comment_score = self.llm_comment_scorer.score_single_completion(response_code)
                
                # 计算可维护性得分
                try:
                    from radon.metrics import mi_visit
                    mi_score = mi_visit(response_code, True)
                    maintainability_score = max(0.0, min(1.0, mi_score / 100.0))  # 保证分数在[0,1]区间
                except Exception as e:
                ...
                reward = {
                    ...
                    "code_comment_score": code_comment_score,
                    "llm_comment_score": llm_comment_score,
                    "comment_score": (code_comment_score + llm_comment_score) / 2,
    ...
    def run(self, save_path: str):
        ...
        with open(self.overall_log_path, "a", encoding="utf-8") as overall_log:
            for idx, obj in enumerate(self.data_list):
                ...
                reward = self.process_question(question_id, overall_log, idx=idx)
                if reward:
                    # 每处理一条数据就追加写入文件
                    with open(self.save_path, 'a', encoding='utf-8') as f:
                        json.dump(reward, f, ensure_ascii=False)
                        f.write('\n')
        return self.rewards
...
if __name__ == "__main__":
    ...
    file_pairs = [
        # (
        #     "/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/grpo_checkpoint740_output.jsonl",
        #     "/data/ytan089/GRPO4CodeGen_v2/dataset/score/grpo_checkpoint740_score_result.jsonl"
        # ),
        (
            "/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/gpt-4.1-mini-2025-04-14_testoutput.jsonl",
            "/data/ytan089/GRPO4CodeGen_v2/dataset/score/gpt-4.1-mini-2025-04-14-Instruct_score_result.jsonl"
        ),
        (
            "/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/Qwen2.5-Coder-7B-Instruct_testoutput.jsonl",
            "/data/ytan089/GRPO4CodeGen_v2/dataset/score/Qwen2.5-Coder-7B-Instruct_score_result.jsonl"
        ),
        # ("/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/grpo_fix_weight_step_testoutput.jsonl",
        #  "/data/ytan089/GRPO4CodeGen_v2/dataset/score/grpo_fix_weight_step_score_result.jsonl"
        # )
        # (
        #     "/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/grpo_use_weight_net_step1776_output.jsonl",
        #     "/data/ytan089/GRPO4CodeGen_v2/dataset/score/grpo_use_weight_net_step1776_score_result.jsonl"
        # )
        # ("/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/grpo_use_weight_net_step2516_output.jsonl",
        #  "/data/ytan089/GRPO4CodeGen_v2/dataset/score/grpo_use_weight_net_step2516_score_result.jsonl"
        # ),
        # ("/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/20250529_all_weightnet_sorting_1e-5_1058.jsonl",
        #  "/data/ytan089/GRPO4CodeGen_v2/dataset/score/20250529_all_weightnet_sorting_1e-5_1058_score_result.jsonl"
        # ),
        # (
        #     "/data/ytan089/GRPO4CodeGen_v2/dataset/grpo_inference_data/20250702_all_data.jsonl",
        #     "/data/ytan089/GRPO4CodeGen_v2/dataset/score/grpo_20250702_all_data_result.jsonl"
        # )
    ]
    execute_code_url = "http://localhost:8895/api/execute_code"

    for jsonl_path, save_path in file_pairs:
        data_list = []
        ...
        print(f"开始评分，输入文件: {jsonl_path}")
        print(len(data_list))
        rewards = tester.run(save_path)
        print(rewards)
